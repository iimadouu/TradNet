...
class TradeExecutor:
    ...
    def check_broker_connection(self) -> Tuple[bool, str]:
        ...
        These flags can be incorrectly reported on some broker setups.
        The actual order_send() will fail gracefully if trading is truly disabled,
        so we don't hard-block here.
        """
        ...
        # Log a warning but don't block - order_send will fail if truly disabled
        if not terminal_info.trade_allowed:
            ...
            # Don't return False - let order_send be the final check
    ...
    def ensure_connection(self, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            if attempt == max_retries - 1:
                ...
                # If it's just a trade permission issue (not connectivity), still return True
                # so we can attempt the trade and let order_send give the real error
    ...
    def place_order(self, signal: Dict) -> bool:
        ...
        # Validate and adjust lot size FIRST
        ...
        # CRITICAL FIX: Don't set TP/SL to 0.0 - some brokers reject/close positions without stops
        # Instead, set very wide safety stops (10x normal) and let bot monitor USD profit
        bot_managed_stops = False
        if self.tp_mode == 'custom' or self.sl_mode == 'custom':
            bot_managed_stops = True
            
            # Set WIDE safety stops (not 0.0) - 10x the normal distance
            ...
            logger.critical(f"[CUSTOM_TP_SET] {symbol}: Custom TP=${self.tp_usd} | MT5 TP={tp_price:.5f} (wide safety)")
            logger.critical(f"[CUSTOM_SL_SET] {symbol}: Custom SL=${self.sl_usd} | MT5 SL={sl_price:.5f} (wide safety)")
            ...
            print(f"   🛡️ MT5 safety stops: TP={tp_price:.5f} SL={sl_price:.5f} (bot will close at custom USD levels)")
    ...
    def check_active_trades(self):
        ...
        for ticket in list(self.active_trades.keys()):
            ...
            # Get position DIRECTLY from MT5 - NO CACHE
            positions = mt5.positions_get()
            if positions is None:
                logger.error(f"[MT5_ERROR] Failed to get positions from MT5")
                continue
            ...
            if position is None:
                ...
                logger.critical(f"[POSITION_NOT_FOUND] {symbol} #{ticket} | Checking if it exists in MT5...")
                
                # Double-check with specific ticket query
                specific_pos = mt5.positions_get(ticket=ticket)
                if specific_pos and len(specific_pos) > 0:
                    position = specific_pos[0]
                    logger.critical(f"[POSITION_FOUND_BY_TICKET] {symbol} #{ticket} | Found using ticket query")
                else:
                    logger.critical(f"[POSITION_CLOSED] {symbol} #{ticket} | Position truly closed")
            ...
            # CRITICAL LOGGING FOR CUSTOM TP/SL
            logger.critical(f"[POSITION_CHECK] {symbol} #{ticket} | Profit_USD=${profit_usd:.2f} | "
                          f"Custom_TP=${self.tp_usd} Custom_SL=${self.sl_usd} | "
                          f"TP_Mode={self.tp_mode} SL_Mode={self.sl_mode}")
            ...
            tick = mt5.symbol_info_tick(symbol)
            if tick is None:
                logger.error(f"[TICK_ERROR] {symbol} #{ticket} | Failed to get tick data")
                continue
            ...
            logger.critical(f"[CUSTOM_TPSL_CHECK] {symbol} #{ticket} | Mode: TP={self.tp_mode} SL={self.sl_mode} | "
                        f"Targets: TP=${self.tp_usd} SL=${self.sl_usd} | Current P&L: ${profit_usd:.2f} ({profit_pips:+.1f}p)")
            
            if self.tp_mode == 'custom' and self.tp_usd:
                tp_hit = profit_usd >= self.tp_usd
                logger.critical(f"[CUSTOM_TP_MONITOR] {symbol} #{ticket} | Current: ${profit_usd:.2f} | Target: ${self.tp_usd} | "
                           f"Distance: ${(self.tp_usd - profit_usd):.2f} | Hit: {tp_hit}")
            ...
            if self.sl_mode == 'custom' and self.sl_usd and not should_close:
                sl_hit = profit_usd <= -self.sl_usd
                logger.critical(f"[CUSTOM_SL_MONITOR] {symbol} #{ticket} | Current: ${profit_usd:.2f} | Limit: -${self.sl_usd} | "
                           f"Distance: ${(profit_usd + self.sl_usd):.2f} | Hit: {sl_hit}")
            ...
            if not should_close and ticket in agent_decisions:
                ...
                # Handle full exit decisions (EXIT_FULL or legacy EXIT_NOW)
                ...
                # Handle SL modification (breakeven/trailing stop)
            ...
            # 3. BOT-MANAGED TP/SL (if custom USD and agent didn't trigger)
            ...
            # 5. TIME-BASED EXITS
            if not should_close:
                # FIXED: use .get() - recovered/old trades may not have 'timestamp' key
                trade_time = trade.get('timestamp', trade.get('entry_time', datetime.now()))
                hold_time = (datetime.now() - trade_time).total_seconds()
